...
public class QueryMatcherTest extends TestCase {
    ...
    private final String RealLineWithSupplierFilterAfterQuery = "INFO: [advantageProductsCollection] webapp=/solr path=/select params={facet=true&sort=ProductNameSort+desc&f.Manufacturer.facet.mincount=1&qf=ProductName^50+ProductDescription^20+CategoryName^1+SubCategoryName^80+ProductNumber+ManfProdNum&version=2.2&rows=25&f.Supplier.facet.mincount=1&defType=dismax&pf=ProductName^25&start=0&facet.query=(PreferredProductContracts:3)&facet.query=(FrequentlyPurchasedFacilities:1456)&q=(vanilla)&group.truncate=true&bf=if(exists(query({!v%3D'PreferredProductContracts:3'})),1,0)^1000+if(exists(query({!v%3D'FrequentlyPurchasedFacilities:1456'})),1,0)^2000&f.SubCategory.facet.mincount=1&facet.field={!ex%3Dc,sc}Supplier&facet.field=Manufacturer&facet.field={!ex%3Dc,sc}Category&facet.field={!ex%3Dsc}SubCategory&f.Category.facet.mincount=1&fq=((Catalogs:27)+OR+(Catalogs:2))&fq=(SupplierID:87)&fq={!tag%3Dc}CategoryID:6695&fq=(FrequentlyPurchasedFacilities:1456)} hits=3 status=0 QTime=4 ";
